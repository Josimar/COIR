...
def xp_rico_clear_bmf(corretora,filename,item,log,page,control):
    ...
    df['Vlr de Operação/Ajuste'] = df['Vlr de Operação/Ajuste'].apply(Utils.funcs.sanitiza_moeda).astype('float')
    ...
    if 'Unnamed: 0' in df_gastos.columns:
        df_gastos['Unnamed: 0'] = df_gastos['Unnamed: 0'].apply(Utils.funcs.sanitiza_nota_bmf)
    ...
    if len(operacoes) == 0 and control == 1:
        ...
        log_move_resultado,pagebmf = Utils.funcs.ove_resultado(folder_path,cpf,nome,item,pagebmf=0)
